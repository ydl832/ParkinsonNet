...
class tcn_unit_attention(nn.Module):
    def __init__(self, in_channels, out_channels, dv_factor, dk_factor, Nh, n,
                 relative, only_temporal_attention, dropout, kernel_size_temporal, stride, multiscale, weight_matrix,
        ...
        if (not self.only_temporal_att):
            self.tcn_conv = Unit2D(in_channels, out_channels - self.dv, dropout=dropout,
                                   ...
                                   stride=self.stride, multiscale=self.multiscale)
    ...
    def forward(self, x):
        ...
        #print(flat_q.shape, flat_k.shape, flat_v.shape)
