...
default_backbone_all_layers = [(3, 64, 1), (64, 64, 1), (64, 64, 1),  (64, 128, 2), (128, 128, 1),
                               (128, 128, 1), (128, 128, 1)]
...
#        ])
    ...
    def forward(self, x, label, name):
        ...
        for i, m in enumerate(self.backbone):
            if i == 2 and self.concat_original:
        ...
        x_q, code_idx, commit_loss = self.quantizer(x_q)
        #x_q = self.sa(x_q)
        #x_q = self.fc(x_q)
        #x = x_q.permute(0, 2, 1)
        # T pooling
        x_q = x_q.permute(0,2,1)
        x_q = F.avg_pool1d(x_q, kernel_size=x_q.size()[2])
        x_q = x_q.squeeze(2)        
        x_q = self.fc(x_q)
#        
#        # C fcn
...
#        
#        x_copy = self.fcn1(x_copy)
#        x_copy = F.avg_pool1d(x_copy, x_copy.size()[2:])
#        x_copy = x_copy.view(N, self.num_class+1)
        
        
                
        # Vector Quantization
#        x_q = x_q.permute(0, 2, 3, 1).contiguous() 
#        x_q = x_q.view(16, 32 * 21, 256)
#        x_q, code_idx, commit_loss = self.quantizer(x_q)
#        x_q = x_q.view(16, 32, 21, 256)
#        x_q = x_q.permute(0, 3, 1, 2).contiguous()

        return x_q, x_q, x_q, commit_loss
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim=128, nhead=4, num_layers=3):
