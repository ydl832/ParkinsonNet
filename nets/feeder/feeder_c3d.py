...
class Feeder_c3d(torch.utils.data.Dataset):
    ...
    def load_data(self, mmap):
        ...
        # load label
        # /home/data/yande/pd/data/poseformerv2_processing/PD_center_False/, PD_train_23.pkl, PD_test_21.pkl, PD_validation_17.pkl
        
        
        data = pickle.load(open(self.data_path, "rb"))
#        val_data = pickle.load(self.data_path, "rb"))
#        train_data = {
...
#        }
        ...
        self.sample_name = data['video_name']
...
#            self.data = np.load(self.data_path)
        ...
        self.data = np.array(self.data).transpose(0, 3, 1, 2)
        self.data = self.data.reshape(self.data.shape[0], self.data.shape[1], self.data.shape[2], self.data.shape[3], 1)
...
#                
        ...
        if self.random_move: # and random.random() > 0.6:
...
#            data_numpy = tools.mirroring_v1(data_numpy)
        if self.mirroring:
            data_numpy = tools.mirror_reflection(data_numpy)
        if self.random_noise:
           data_numpy = tools.random_noise(data_numpy)
        if self.axis_mask:
           data_numpy = tools.axis_mask(data_numpy)
        if self.joint_dropout:
           data_numpy = tools.joint_dropout(data_numpy)
#        if self.distance:
