        ...
        if (arg.validation_split):
            ...
            self.trainLoader, self.valLoader = torch.utils.data.random_split(self.trainLoader,
                                                                             [len(self.trainLoader) - val_size,
                                                                              val_size])
        print("Train size: ", len(self.trainLoader))
        print("Test size: ", len(self.testLoader))
...
#        self.data_loader['comp'] = torch.utils.data.DataLoader(
#            dataset=self.compareLoader,
#            batch_size=4,
#            shuffle=False,
#            num_workers=self.arg.num_worker)
...
#            lr = self.arg.base_lr * (1 + np.cos(np.pi * epoch / self.arg.num_epoch)) / 2
    ...
    def train(self, epoch, save_model=True):
        ...
        for batch_idx, (data, label, name) in enumerate(loader):
            ...
            output_vq, feature, output_ce, output_se, commit_loss= self.model(data, label, name)
...
#                    loss_vq = ordinal(output_vq,label)
                ...
                conf_matrix_val += confusion_matrix(preds_vq.cpu(), label.cpu(), labels=np.arange(4))
...
#                writer.add_scalar(tag, value, step)
        ...
        return val_accuracy, val_accuracy_1, labels, preds_sm_labels, preds_vq_labels
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if eval_model:
                    accuracy, accuracy_1, labels, preds_sm_labels, preds_vq_labels = self.val(
                        epoch,
                    ...
                    if (accuracy <= self.best_accuracy):
                        ...
                        np.save('./checkpoints/y_true.npy', labels.cpu())
                        np.save('./checkpoints/y_sm.npy', preds_sm_labels.cpu())
                        np.save('./checkpoints/y_vq.npy', preds_vq_labels.cpu())
                ...
                print('best_acc:',self.best_accuracy, 'best_epoch:', self.best_epoch+1)
            print('best_acc:',self.best_accuracy, 'best_epoch:', self.best_epoch+1)
            
            file = open('/home/yande/ST-TR/best_acc.txt', 'a')
            file.write(str(self.best_accuracy) + ' ' + str(self.best_epoch+1))
            file.write('\n')
            file.close()
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        set_seed(13696641)  # ordical loss
